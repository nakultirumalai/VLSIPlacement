...
my $i=0;
print outFile "# Pseudo_name\tCell_name\tLibrary_name\tStandard_cell_name\tType\n";
while(my $line = <inFile>) {
    next if($line =~ m/^(#|\n|NumNodes|NumTerminals|UCLA)/);
    if ($line =~m/\s*(.*)\s*PAD\s*$/) {
	my $cell_name = $1; 
	print outFile "o$i\t$cell_name -- -- PAD\n";
	$i++;
	next;
    } 
    chomp($line);
    ...
    print outFile "o$i\t$cell_name\t$splitlib_names[0]\t$splitlib_names[1]\t$type\n";
    $i++;
}
close(outFile);
close(inFile);
