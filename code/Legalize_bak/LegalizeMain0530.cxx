...
LegalizeAugPathAlgo(HyperGraph &myGraph, LegalizeBin *suppNode, vector<LegalizeBin *> &PathBins)
{
  ...
  if (debugMode) {
    ...
    cout << "SOURCE BIN " << endl;
    LegalizePrintBinData(suppNode);
  ...
  uint numNodes = myGraph.HyperGraphGetNumNodes();
  cout << "TOTAL NUM OF NODES IN DESIGN " << numNodes << endl;
  cout << "TOTAL NUM OF EDGES IN DESIGN " << numEdges <<  endl;
...
/* Get all the physical rows in Design */
  ...
  VECTOR_FOR_ALL_ELEMS(DesignBins, vector<LegalizeBin*>, binsInRow) {
    VECTOR_FOR_ALL_ELEMS(binsInRow, LegalizeBin*, thisBin) {
      binX = (*thisBin).BinGetBegin();
      cellsInBin = (*thisBin).BinGetCellsInBin();
      sort(cellsInBin.begin(), cellsInBin.end(), ascendingX);
      bool firstCell = true;
      VECTOR_FOR_ALL_ELEMS(cellsInBin, Cell*, thisCell){
	cellWidth = (*thisCell).CellGetWidth();
	if (firstCell) {
	  (*thisCell).CellSetXpos(binX);
	  nextCellBegin = binX + cellWidth;
	  firstCell = false;
	} else {
