...
# endif
  ...
  while(1) {
	...
	//	LegalizeBin *thisNode = currentNode;
	currentNode = binLValObj.first;
	//	pathMap[currentNode] = thisNode;
	if (0) {
  ...
  if (dest != NIL(LegalizeBin*)) {  
    if (0) {
      PhysRow *thisRow = (*dest).BinGetRow();
      
      cout << "DESTINATION := " << (*thisRow).PhysRowGetIndex() << "[" << (*dest).BinGetIndex() << "]" << endl;
    ...
    if (0) {
      LegalizeBin *thisBin;
      VECTOR_FOR_ALL_ELEMS(reversePath, LegalizeBin*, thisBin) {
	PhysRow *thisRow = (*thisBin).BinGetRow();
	cout << (*thisRow).PhysRowGetIndex() << "[" << (*thisBin).BinGetIndex() << "] <-- ";
      } END_FOR;
      cout << endl;
    }
...
/* Get all the physical rows in Design */
  ...
  */
  ...
  while (supplyHeap.size()) {
    cout << "TOP ITERATION: # " << iteration << endl;
    if (0) {
      ...
      int presSupply;
      LegalizeBin *thisBin;
      VECTOR_FOR_ALL_ELEMS(supplyHeap, LegalizeBin*, thisBin){
	presBinIdx = (*thisBin).BinGetIndex();
	presRow = (*thisBin).BinGetRow();
	presRowIdx = (*presRow).PhysRowGetIndex();
	presSupply = (*thisBin).BinGetSupply();
    ...
    */
    ...
    //    LegalizePlotData("supply and demand bins", "supplytest_2.plt", myDesign, DesignBins, PathBins);

    make_heap(supplyHeap.begin(), supplyHeap.end(), maxSupply);
    maxSuppBin = supplyHeap.front();
    pop_heap(supplyHeap.begin(), supplyHeap.end(), maxSupply);
    supplyHeap.pop_back();
    cout << "SUPPLY HEAP SIZE " << supplyHeap.size() << endl;
    LegalizeAssignLValues(DesignBins);
    iteration++;
